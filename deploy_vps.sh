...
# ==============================================================================
# Trrabb Platform - Automated VPS Production Deployment Script
# ==============================================================================
set -e
...
echo "📦 [2/5] Building Admin Panel (admin)..."
cd admin
...
cd frontend
export NEXT_PUBLIC_API_URL="${NEXT_PUBLIC_API_URL:-https://api.trrabb.com/api}"
npm install --no-audit
npm run build
cd ..
...
pm2 restart trrabb-backend --update-env || true
if pm2 describe trrabb-web >/dev/null 2>&1; then
  pm2 reload trrabb-web --update-env
else
