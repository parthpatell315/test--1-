...
import { Compass, Plus, Eye, Pencil, Trash2 } from "lucide-react";
...
export function ActivitiesModuleView({
  ...
  const activities = vendors.filter(
    (v) => (v.type || "").toUpperCase() === "ACTIVITIES",
  );
  ...
  return (
    <div className="space-y-4">
      <VendorSearchToolbar
        ...
        placeholder="Search activity, experience, or city"
      ...
      <div className="overflow-hidden rounded-xl border border-[#E8EEF4] bg-white">
        <table className="w-full border-collapse text-left text-xs">
          <thead className="border-b border-[#E8EEF4] bg-[#F8FAFC] text-[11px] font-semibold text-slate-500">
            <tr>
              <th className="py-3 px-4">Activity Vendor</th>
                ...
                <td colSpan={5} className="p-12 text-center bg-slate-50/50">
                  <div className="space-y-2 max-w-sm mx-auto">
                    ...
                    <p className="text-sm font-bold text-slate-800">
                      No Activity Vendors Found
                    </p>
                    <p className="text-xs text-slate-500">
                      There are no activity partners registered in this scope. Click below to add an activity vendor.
                    </p>
                    <Button
                      onClick={onAddVendor}
                    ...
                    >
                      <Plus className="w-4 h-4" /> Add Activity Vendor
                    </Button>
                  </div>
                </td>
              </tr>
                ...
                >
                  <td className="py-2.5 px-4">
                    <div className="flex items-center gap-2.5">
                      <div className="w-7 h-7 rounded-md bg-[#FF4D00]/5 border border-[#FF4D00]/30 flex items-center justify-center text-[#C2410C] font-bold shrink-0">
                        <Compass className="w-3.5 h-3.5" />
                      </div>
                      <div>
                        <span
                          onClick={() => onSelectVendor(a)}
                  ...
                  <td className="py-2.5 px-4 whitespace-nowrap text-slate-700 font-medium text-xs">
                    📍 {a.city || a.location || "N/A"}
                  </td>
                  <td className="py-2.5 px-4 whitespace-nowrap">
                    <span className="font-bold text-slate-800 text-xs block">
                      {a.contactPerson || a.name}
                    </span>
                    <span className="text-[11px] font-mono text-slate-500">
                      {a.phone || a.contactNumber || "—"}
                    </span>
                  </td>
      ...
      <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-200 text-xs text-slate-600 font-medium">
        <span>
          ...
          {pagination?.total ?? 0} activity vendors
        </span>
